		 ...
		 if ($row = mysql_fetch_array($result))
			{
			 ...
			 do
				{ 
					if(($row["nombre"] == $user) && ($row["contrasena"] == md5($pass)))
				     {
...
function cambiar_contra($x,$user,$pass,$nueva)	
{  
	    ...
	    $sql="SELECT * FROM usuarios";
		 ...
		 {  
		    if ($row["contrasena"] == md5($pass))
		       {
                  $consult= "UPDATE usuarios SET 
						                 nombre='".$user."',
										 contraseña='".$nueva."',
										WHERE id_usuario=$x ";
		// $res4=mysql_query($consult) or die(mysql_error());
					  
	
			     mysql_query($consult,$var->links);
...
function valida($user , $pass, $nueva)
	{
		  $sql="SELECT * FROM usuarios";
	...
	// $x=buscar_usuario($user,$pass); //aca guarda el id de usuario que coincida con esa contraseña 
	 ...
	 if($x==0)	
		 ...
		 //header("Location: no_login.php");
			...
			// inicia_sesion1($x,$user,$pass); 
			 cambiar_contra($x,$user,$pass,$nueva);
			}
...
switch($_POST['oper'])
	{
	 case 'W': { 
	 			...
	 			valida($_POST['user'],$_POST['pass'],$_POST['nueva'] );
				break;
				}
